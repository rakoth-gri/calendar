...
#### ВАЖНО! объект **options** манипулирует с inline-стилями (CSS-специфичность: **1, 0, 0, 0**). Если, Вы захотите по ходу алгоритма переопределить их или добавить к ним новые - воспользуйтесь методами API (рассмотрим далее), либо вовсе не передавайте их.
...
#### Темизация:
#### Метод переключает со светлой на темную и наоборот:
...
+ ###### styles - строка стилей в СSS-нотации либо пустая строка:
  - **'text-transform: uppercase; background: white;'**
 
```javascript

// 1 аргумент - строка Селектора, inline-стили которого хотим обновить или добавить впервые:
// Метод не удаляет имеющиеся у Селектора inline-стили; добавляет к ним новые или перезаписывает одноименные!
